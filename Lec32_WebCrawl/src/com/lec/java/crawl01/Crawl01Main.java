 ...
 * 3. 프로젝트 우클릭 - Configure Build Path - Library탭 - Add Jars (혹은 라이브러리 추가)
 *                 - 추가할 라이브러리 선택
 */
...
//		403  권한에러 Forbidden : 파일권한, 읽기권한, SSL, IP, 등...  <--- 웹 크롤링 하다가 은근히 자주 마주치게 되는 에러
		...
		doc = response.parse();
		System.out.println(doc.title()); // <title> element
...
//		System.out.println(outerHtml);
		...
		//select(), selectFirst() 메소드로특정element(들)을 추출
		
		System.out.println("[네이버 뉴스 캐스트]");
		
		element = doc.selectFirst("#news_cast"); // 검색된 element 들 중 최초 한개
